...
DATABASES = {
    db_profile: {
        key: getenv("%s:%s" % (db_profile, key))
        for key in getenv("%s:KEYS" % db_profile).split(",")
    }
    for db_profile in getenv("DB").split(",")
}

DATABASES["default"] = DATABASES.get(getenv("DB_DEFAULT"))
